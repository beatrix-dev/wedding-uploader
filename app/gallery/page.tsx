...
export default function GuestGallery() {
  ...
  const handleDelete = async (key: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Failed to delete. Check if the API route exists!");
      }
    } catch (err) {
  ...
  if (loading) return <div className="text-center p-20 font-serif">Loading memories... ✨</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
          ...
          <div key={photo.key} className="relative group break-inside-avoid rounded-xl overflow-hidden shadow-lg hover:scale-[1.02] transition-transform bg-black">
            ...
            {myUploads.includes(photo.key) && (
              <button
                ...
                className="absolute top-2 right-2 bg-white/90 hover:bg-red-500 hover:text-white text-gray-800 p-2 rounded-full shadow-md transition-colors z-10"
            ...
            {/* DEBUG LINE: Hover over a photo on your computer to see if 'Owned' says Yes */}
            <span className="text-[10px] text-white hidden group-hover:block absolute bottom-0 left-0 right-0 bg-black/70 p-1 text-center">
               Owned: {myUploads.includes(photo.key) ? '✅ Yes' : '❌ No'}
            </span>
          </div>
        ))}
      </div>
      ...
      {photos.length === 0 && (
        <p className="text-center text-gray-500 mt-10">No photos yet. Be the first to upload! 📸</p>
      )}
    </div>
  );
}